<x-app-layout>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
                ...
                <div class="max-w-xl">
                    ...
                    <dl class="grid grid-cols-1 gap-x-4 gap-y-6 sm:grid-cols-2">
                        <div class="sm:col-span-1">
                            ...
                            <dd class="mt-1 text-sm text-gray-900">{{ $meetingLog->title }}</dd>
                        </div>
                        <div class="sm:col-span-1">
                            ...
                            <dd class="mt-1 text-sm text-gray-900">
                                @if($meetingLog->youtube_url)
                                    <a href="{{ $meetingLog->youtube_url }}" target="_blank"
                                        class="text-indigo-600 hover:text-indigo-900">視聴する</a>
                                @else
            ...
            <!-- Transcript Section (Teacher/Admin Only) -->
                ...
                <div class="p-4 sm:p-8 bg-white shadow sm:rounded-lg">
                    <div class="max-w-4xl">
                        ...
                        <form action="{{ route('teacher.meeting-logs.transcript.upload', $meetingLog) }}" method="POST"
                            ...
                            @csrf
                            <div>
                                <x-input-label for="transcript_file" value="字幕ファイル (.srt)" />
                                <input type="file" name="transcript_file" id="transcript_file" accept=".srt" class="block w-full text-sm text-slate-500
                        ...
                        <div class="mt-8 pt-8 border-t border-gray-200">
                            <div class="flex items-center justify-between mb-4">
                                ...
                                @if($meetingLog->transcript_status === 'ready')
                                    <form action="{{ route('teacher.meeting-logs.summarize', $meetingLog) }}" method="POST">
                                        @csrf
                                        <button type="submit"
                            ...
                            <form action="{{ route('teacher.meeting-logs.update', $meetingLog) }}" method="POST">
                                ...
                                <input type="hidden" name="title" value="{{ $meetingLog->title }}">
                                <input type="hidden" name="started_at" value="{{ $meetingLog->started_at }}">
